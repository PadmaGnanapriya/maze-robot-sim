...
export default function RobotSettings() {
  ...
  const num = (key, label, unit, min, max, step) => (
    <NumberField key={key} label={label} unit={unit} min={min} max={max} step={step} value={R[key]} onCommit={v => sim.setRobot({ [key]: v })} />
  );
  const parts = [];
  ...
  return (
    <div className="scroll form">
      ...
      {parts.length > 0 && <SyncCallout onSync={() => sim.syncConfig('robot')}>config.h has {parts.reduce((a, p, i) => (i ? [...a, ' and ', p] : [p]), [])}.</SyncCallout>}
