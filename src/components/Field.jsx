...
export function NumberField({ label, value, unit, min, max, step = 1, onCommit, help }) {
  const id = useId();
...
export function SelectField({ label, value, options, onChange }) {
  ...
  return (
    <div className="field">
      <label htmlFor={id}>{label}</label>
      <div className="in">
        <select id={id} className="sel" value={value} onChange={e => onChange(typeof options[0][0] === 'number' ? +e.target.value : e.target.value)}>
...
export function SyncCallout({ children, onSync }) {
  return (
