...
import { SyncCallout } from './Field.jsx';

const pinName = p => (p < 14 ? 'D' + p : 'A' + (p - 14));
const PIN_OPTIONS = Array.from({ length: 20 }, (_, p) => [p, pinName(p) + (PWM_PINS.has(p) ? ' (PWM)' : '')]);
...
export default function WiringSettings() {
  ...
  const uses = {};
  const note = (p, name) => { (uses[p] ||= []).push(name); };
  ['ENA', 'IN1', 'IN2', 'IN3', 'IN4', 'ENB'].forEach(k => note(W[k], k));
  W.sonars.forEach(s => { note(s.trig, s.id + ' trig'); note(s.echo, s.id + ' echo'); });
  const clash = p => uses[p]?.length > 1;
  ...
  const rows = [
    ...
    ...W.sonars.flatMap((s, i) => [
      [SONAR_NAMES[i], 'TRIG', s.trig, v => sim.setWiring(w => { w.sonars[i].trig = v; })],
      [SONAR_NAMES[i], 'ECHO', s.echo, v => sim.setWiring(w => { w.sonars[i].echo = v; })],
    ]),
  ];
