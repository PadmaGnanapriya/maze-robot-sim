...
import type { SolvedResult } from '../sim/SimController.js';

export default function SolvedBanner() {
  const sim = useSimController();
  const { runState, solved, mazeSize } = useSimState();
  const [dismissed, setDismissed] = useState<SolvedResult | null>(null);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setDismissed(solved); };
    document.addEventListener('keydown', onKey);
    return () => document.removeEventListener('keydown', onKey);
  }, [open, solved]);
  if (!open || !solved) return null;
