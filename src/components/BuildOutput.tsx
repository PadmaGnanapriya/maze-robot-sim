...
import type { OutputKind } from '../sim/SimController.js';

const KIND_LABEL: Partial<Record<OutputKind, string>> = { error: 'error:', warning: 'warning:', fault: 'stopped:' };
      ...
      <div className="out-list" ref={list} aria-live="polite">
        ...
        {output.map(e => (
          <div className={'msg ' + e.kind} key={e.id}>
            <i className="ic" />
            <div>
              {e.file && e.line ? <button className="loc" onClick={() => sim.jumpTo(e.file!, e.line!, e.col ?? 0)}>{e.file}:{e.line}{e.col ? ':' + e.col : ''}</button> : null}{' '}
