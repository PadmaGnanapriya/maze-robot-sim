 ...
 */
export default function CodeEditor({ value, onChange, marks, jump, label }) {
  const ta = useRef(null), pre = useRef(null), gutter = useRef(null), flags = useRef(null);
  ...
  useEffect(() => {
    if (!jump) return;
    const t = ta.current, lines = t.value.split('\n');
    const line = Math.min(Math.max(1, jump.line | 0), lines.length);
    let pos = 0; for (let i = 0; i < line - 1; i++) pos += lines[i].length + 1;
    const col = Math.min(Math.max(0, (jump.col | 0) - 1), lines[line - 1].length);
    t.focus({ preventScroll: true });
    t.setSelectionRange(pos + col, pos + lines[line - 1].length);
  ...
  const onKeyDown = e => {
    const t = ta.current, v = t.value, s = t.selectionStart, en = t.selectionEnd;
