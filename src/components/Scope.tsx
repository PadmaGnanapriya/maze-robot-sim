...
import type { SonarReading } from '../sim/SimController.js';
...
export default function Scope({ readings, now }: { readings: (SonarReading | null)[]; now: number }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const history = useRef<[number, number | null][][]>([[], [], []]), lastT = useRef([-1, -1, -1]);
  ...
  useEffect(() => {
    ...
    const y = (d: number) => h - 3 - Math.sqrt(Math.min(150, d) / 150) * (h - 8);
    const t0 = now - SPAN;
