...
import type { PanelTab } from '../sim/settings.js';
import SketchTab from './SketchTab.js';
import SerialMonitor from './SerialMonitor.js';
import RobotSettings from './RobotSettings.js';
import MazeSettings from './MazeSettings.js';
import WiringSettings from './WiringSettings.js';

const TABS: [PanelTab, string][] = [['sketch', 'Sketch'], ['serial', 'Serial monitor'], ['robot', 'Robot'], ['maze', 'Maze'], ['wiring', 'Wiring']];
...
export default function Panel({ open }: { open: boolean }) {
  ...
  const select = (id: PanelTab) => sim.setUi({ tab: id });
