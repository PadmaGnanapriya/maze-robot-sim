...
/**
 * Escapes every character that could open an HTML tag or attribute before it reaches
 * `dangerouslySetInnerHTML` in CodeEditor.tsx. `highlight()` must never emit raw source
 * text - only this escaped form or `<span class="tk-...">` around escaped text - or the
 * editor becomes an XSS sink for whatever the visitor (or a pasted/loaded file) types.
 */
