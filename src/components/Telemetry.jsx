...
const SONARS = [['Front', 'var(--sF)'], ['Left', 'var(--sL)'], ['Right', 'var(--sR)']];
const pinName = p => (p < 14 ? 'D' + p : 'A' + (p - 14));

function pinRoles(w) {
  const roles = {};
  const add = (p, r) => { roles[p] = roles[p] ? roles[p] + ', ' + r : r; };
  add(w.ENA, 'ENA left speed'); add(w.IN1, 'IN1'); add(w.IN2, 'IN2'); add(w.IN3, 'IN3'); add(w.IN4, 'IN4'); add(w.ENB, 'ENB right speed');
  ['front', 'left', 'right'].forEach((n, i) => { add(w.sonars[i].trig, n + ' sonar TRIG'); add(w.sonars[i].echo, n + ' sonar ECHO'); });
...
const Telemetry = forwardRef(function Telemetry(_, ref) {
  ...
  const hitsRef = useRef(null), lastHits = useRef(t.contacts);
  useEffect(() => {
            ...
            <div className="pins">
              {t.pins.map((p, i) => (
                <div key={i} title={pinName(i) + (roles[i] ? ': ' + roles[i] : '')}
                  ...
                  style={{ '--lv': p.level }}>{pinName(i)}</div>
              ))}
            </div>
