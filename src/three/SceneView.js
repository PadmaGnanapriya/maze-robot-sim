...
import { buildMazeGroup, placeGoalMesh } from './mazeMesh.js';
import { buildRobotModel } from './robotModel.js';
...
export class SceneView {
   ...
   * @param {import('../sim/SimController.js').SimController} sim
   * @param {{ reduceMotion: boolean, reservedLeft: () => number }} options
   */
  constructor(canvas, sim, { reduceMotion, reservedLeft }) {
    this.canvas = canvas;
    this.sim = sim;
  ...
  setView(mode) { this.rig.setMode(mode); }
  ...
  fit() {
    ...
    this.scene.fog.near = fog.fogNear; this.scene.fog.far = fog.fogFar;
  ...
  bindPointer() {
    ...
    const P = { mode: null, x: 0, y: 0, pointers: new Map(), offset: null, paint: 0, lastEdge: null, pinchD: 0, pinchMid: null };
    const aim = e => { const r = c.getBoundingClientRect(); ndc.set((e.clientX - r.left) / r.width * 2 - 1, -(e.clientY - r.top) / r.height * 2 + 1); ray.setFromCamera(ndc, this.camera); };
    const floorAt = e => { aim(e); return ray.ray.intersectPlane(ground, hitPt) ? { x: hitPt.x, y: -hitPt.z } : null; };
    const pick = e => {
      aim(e);
    ...
    const setCursor = cur => { c.style.cursor = cur; };
    const ghost = edge => this.ghost.show(edge, sim.mz, edge ? (P.mode === 'paint' ? P.paint === 0 : sim.edgeValue(edge) === 1) : false);
    ...
    const move = e => {
      ...
      if (!P.mode) {   // hover feedback
        const hit = pick(e);
        this.robot.ring.material.opacity = hit === 'robot' ? 0.55 : 0;
    ...
    const leave = () => { if (!P.mode) { this.ghost.show(null); this.robot.ring.material.opacity = 0; } };
