...
export class SonarBeams {
  ...
  update(world, sensorHeight, { show, pov }) {
...
export class Trail {
  ...
  update(points, show) {
    ...
    const pos = this.geo.attributes.position, end = Math.min(points.length, this.max);
    for (let i = this.n; i < end; i++) pos.setXYZ(i, points[i].x, 0.12, -points[i].y);
    this.n = end; this.first = points[0];
