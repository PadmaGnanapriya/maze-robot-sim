...
export class CameraRig {
  constructor(camera, reduceMotion) {
    this.camera = camera;
    this.reduceMotion = reduceMotion;
    this.mode = 'orbit';
    this.target = new THREE.Vector3();      // orbit pivot
    this.topTarget = new THREE.Vector3();   // plan view centre
    this.dist = 400; this.az = -0.5; this.el = 0.95;
    this.topDist = 400;
    this.follow = { az: 0, el: 0.82, dist: 80 };
    this.pov = { yaw: 0, pitch: -0.12 };
    this.pos = new THREE.Vector3(); this.look = new THREE.Vector3(); this.up = new THREE.Vector3(0, 1, 0);
    this.snapNext = true;
    this._p = new THREE.Vector3(); this._l = new THREE.Vector3(); this._u = new THREE.Vector3();
  }
   ...
   */
  fit(mazeW, mazeH, viewW, viewH, reserveLeft = 0) {
  ...
  desired(world, sensorHeight, pos, look) {
    const p = world.pose;
    switch (this.mode) {
      case 'top':
