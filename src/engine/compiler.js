...
import { ARDUINO_CONSTS, BUILTINS, CAST_NAMES, GEN_BUILTINS, GEN_METHODS, INT_K, NEWPING_METHODS, PWM_PINS, SERIAL_METHODS, SIG_BRK, SIG_CNT, SIG_RET, STRING_METHODS, TY, WRAP, arrT, cloneVal, commonK, fmtValue, fr, isNumK, kindOf, promoteK, sizeOfT, subT, toStrFn, typeName } from './types.js';
...
class ArduinoCompiler {
  constructor(ast, M, mainFile) {
    this.ast = ast; this.M = M; this.mainFile = mainFile;
    this.G = []; this.gscope = new Map(); this.funcs = new Map(); this.enumScopes = new Map();
    this.warnings = []; this.initSteps = []; this.globalRam = 0; this.strRam = 0; this.usesSerial = false; this.fn = null;
    this.structsDone = new Set();
  }
  err(msg, tok) { throw new CompileError(msg, tok || { file: this.mainFile, line: 0, col: 0 }); }
  warn(msg, tok) {
  ...
  K(t, v) { return { t, s: true, f: () => v, cv: v }; }
  lookup(name) {
  ...
  curScope() { return this.fn ? this.fn.scopes[this.fn.scopes.length - 1] : this.gscope; }
  ...
  ensureStruct(def) {
    ...
    def.cfields = def.fields.map(f => {
      ...
      return { name: f.name, t, initConst, tok: f.tok };
    });
  ...
  zeroOf(t) {
    switch (t.k) {
      case 'array': { const n = t.dims[0], st = subT(t); const a = new Array(n); for (let i = 0; i < n; i++) a[i] = this.zeroOf(st); return a; }
      case 'struct': { const o = {}; for (const f of t.def.cfields) o[f.name] = f.initConst !== undefined ? cloneVal(f.initConst) : this.zeroOf(f.t); return o; }
      case 'str': return '';
      case 'obj': return null;
      default: return 0;
    }
  }
  convFn(from, to, tok, explicit) {
    ...
    if (to.k === 'array' || from.k === 'array') {
      ...
      if (to.k === 'str' && from.k === 'array' && (from.elem.k === 'i8' || from.elem.k === 'u8')) return a => String.fromCharCode(...a.filter(c => c)).split('\0')[0];
    ...
    if (fk === 'str') this.err(`cannot convert 'String' to '${typeName(to)}'${from.charPtr ? '' : ' (use .toInt() or .toFloat())'}`, tok);
  ...
  constInt(expr, what) {
    ...
    if (!INT_K.has(kindOf(E.t))) this.err(`${what} must be an integer, not '${typeName(E.t)}'`, expr.tok);
    return E.cv;
  }
  ...
  compileExpr(n) {
    switch (n.k) {
      ...
      case 'Binary': {
        ...
        if ((n.op === '/' || n.op === '%') && B.cv === 0 && INT_K.has(kindOf(B.t))) this.warn('division by zero [-Wdiv-by-zero]', n.tok);
      ...
      case 'Comma': {
        ...
        if (list.every(x => x.s)) return { t: last.t, s: true, f: F => { let v; for (let i = 0; i < list.length; i++) v = list[i].f(F); return v; } };
    ...
    this.err('unsupported expression', n.tok);
  }
  compileNum(n) {
    const tk = n.tok;
  ...
  compileIdent(n) {
    ...
    switch (sym.kind) {
      ...
      case 'local': { if (sym.cv !== undefined) return this.K(sym.t, sym.cv); const s = sym.slot; return { t: sym.t, s: true, f: F => F[s] }; }
      case 'ref': { const s = sym.slot; return { t: sym.t, s: true, f: F => { const p = F[s]; return p[0][p[1]]; } }; }
      case 'serial': this.usesSerial = true; return { t: { k: 'serialObj' }, s: true, f: () => 1 };
    }
    this.err(`cannot use '${n.name}' here`, n.tok);
  }
  compileQual(n) {
  ...
  combine2(rt, A, B, fn, tok, op) {
    ...
    const fa = A.f, fb = B.f;
    if (A.s && B.s) return { t: rt, s: true, f: F => fn(fa(F), fb(F)) };
    const as = A.s, bs = B.s;
    return { t: rt, s: false, f: function* (F) { const a = as ? fa(F) : yield* fa(F); const b = bs ? fb(F) : yield* fb(F); return fn(a, b); } };
  }
  binOp(op, ta, tb, tok) {
    const ka0 = kindOf(ta), kb0 = kindOf(tb);
    const scalar = k => isNumK(k) || k === 'str';
    ...
    if (ka === 'str' || kb === 'str') {
      if (op === '+') {
        if (ta.lit && tb.lit) this.err(`invalid operands of types 'const char*' to binary 'operator+' (wrap one side in String(...))`, tok);
        const f1 = toStrFn(ta), f2 = toStrFn(tb); return { rt: TY.str, fn: (a, b) => f1(a) + f2(b) };
      }
      if (ka === 'str' && kb === 'str') {
        const cmp = { '==': (a, b) => (a === b ? 1 : 0), '!=': (a, b) => (a !== b ? 1 : 0), '<': (a, b) => (a < b ? 1 : 0), '>': (a, b) => (a > b ? 1 : 0), '<=': (a, b) => (a <= b ? 1 : 0), '>=': (a, b) => (a >= b ? 1 : 0) }[op];
    ...
    switch (op) {
      case '+': case '-': case '*': {
        if (ct === 'f32') return { rt: T, fn: op === '+' ? (a, b) => fr(a + b) : op === '-' ? (a, b) => fr(a - b) : (a, b) => fr(a * b) };
        if (ct === 'i64') return { rt: T, fn: op === '+' ? (a, b) => a + b : op === '-' ? (a, b) => a - b : (a, b) => a * b };
        if (ct === 'i32') return { rt: T, fn: op === '+' ? (a, b) => (a + b) | 0 : op === '-' ? (a, b) => (a - b) | 0 : (a, b) => Math.imul(a, b) };
        if (ct === 'u32') return { rt: T, fn: op === '+' ? (a, b) => (a + b) >>> 0 : op === '-' ? (a, b) => (a - b) >>> 0 : (a, b) => Math.imul(a, b) >>> 0 };
        const w = WRAP[ct];
        return { rt: T, fn: op === '+' ? (a, b) => w(a + b) : op === '-' ? (a, b) => w(a - b) : (a, b) => w(a * b) };
      }
      case '/': {
        if (ct === 'f32') return { rt: T, fn: (a, b) => fr(a / b) };
        const w = WRAP[ct];
        return { rt: T, fn: (a, b) => { a = w(a); b = w(b); if (b === 0) { M.divZero(tok); return w(-1); } return w(Math.trunc(a / b)); } };
      }
      case '%': {
        if (ct === 'f32') this.err(`invalid operands of types '${typeName(ta)}' and '${typeName(tb)}' to binary 'operator%' (use fmod() for floats)`, tok);
        const w = WRAP[ct];
        return { rt: T, fn: (a, b) => { a = w(a); b = w(b); if (b === 0) { M.divZero(tok); return 0; } return w(a % b); } };
      }
      case '<<': case '>>': {
        if (ka === 'f32' || kb === 'f32') this.err(`invalid operands of types '${typeName(ta)}' and '${typeName(tb)}' to binary 'operator${op}'`, tok);
        const rk = ka, R = TY[rk];
        if (op === '<<') {
          if (rk === 'u32') return { rt: R, fn: (a, b) => (b >= 32 ? 0 : (a << b) >>> 0) };
          if (rk === 'i32') return { rt: R, fn: (a, b) => (b >= 32 ? 0 : a << b) };
          if (rk === 'i64') return { rt: R, fn: (a, b) => a * Math.pow(2, b) };
          const w = WRAP[rk]; return { rt: R, fn: (a, b) => (b >= 32 ? 0 : w(a << b)) };
        }
        if (rk === 'u32') return { rt: R, fn: (a, b) => (b >= 32 ? 0 : a >>> b) };
        if (rk === 'i64') return { rt: R, fn: (a, b) => Math.floor(a / Math.pow(2, b)) };
        return { rt: R, fn: (a, b) => (b >= 32 ? (a < 0 ? -1 : 0) : a >> b) };
      }
      case '&': case '|': case '^': {
      ...
      case '==': case '!=': case '<': case '>': case '<=': case '>=': {
        ...
        const W2 = need ? w : ID;
        switch (op) {
          case '==': return { rt: TY.bool, fn: (a, b) => (W2(a) === W2(b) ? 1 : 0) };
          case '!=': return { rt: TY.bool, fn: (a, b) => (W2(a) !== W2(b) ? 1 : 0) };
          case '<': return { rt: TY.bool, fn: (a, b) => (W2(a) < W2(b) ? 1 : 0) };
          case '>': return { rt: TY.bool, fn: (a, b) => (W2(a) > W2(b) ? 1 : 0) };
          case '<=': return { rt: TY.bool, fn: (a, b) => (W2(a) <= W2(b) ? 1 : 0) };
  ...
  compileLogical(n) {
    ...
    if (as && bs) return { t: TY.bool, s: true, f: and ? F => (fa(F) && fb(F) ? 1 : 0) : F => (fa(F) || fb(F) ? 1 : 0) };
    return {
      t: TY.bool, s: false, f: function* (F) {
        const a = as ? fa(F) : yield* fa(F);
        if (and ? !a : a) return and ? 0 : 1;
        const b = bs ? fb(F) : yield* fb(F);
        return b ? 1 : 0;
      }
    };
  }
  ...
  compileLv(n, tok, what) {
    ...
    if (n.k === 'Ident') {
      ...
      if (sym.isConst) this.err(`assignment of read-only variable '${n.name}'`, n.tok);
      const s = sym.slot;
      if (sym.kind === 'global') return { t: sym.t, fast: true, s: true, get: () => G[s], set: (F, v) => { G[s] = v; }, pair: () => [G, s] };
      if (sym.kind === 'local') return { t: sym.t, fast: true, s: true, get: F => F[s], set: (F, v) => { F[s] = v; }, pair: F => [F, s] };
      if (sym.kind === 'ref') return { t: sym.t, fast: true, s: true, get: F => { const p = F[s]; return p[0][p[1]]; }, set: (F, v) => { const p = F[s]; p[0][p[1]] = v; }, pair: F => F[s] };
    }
    if (n.k === 'Index') {
      ...
      if (!INT_K.has(kindOf(I.t))) this.err(`invalid types '${typeName(B.t)}[${typeName(I.t)}]' for array subscript`, n.tok);
      const t = subT(B.t), fb = B.f, fi = I.f, itok = n.tok;
      const scratch = [0];
      const pick = (a, i) => { if (i >= 0 && i < a.length) return [a, i]; M.oob(itok, i, a.length); scratch[0] = 0; return [scratch, 0]; };
      if (B.s && I.s) return { t, fast: false, s: true, loc: F => pick(fb(F), fi(F)) };
      const bs = B.s, is = I.s;
      return { t, fast: false, s: false, loc: function* (F) { const a = bs ? fb(F) : yield* fb(F); const i = is ? fi(F) : yield* fi(F); return pick(a, i); } };
    }
    if (n.k === 'Member') {
      const B = this.compileExpr(n.obj);
      const fld = this.fieldOf(B.t, n.name, n.tok);
      const fb = B.f, nm = n.name;
      if (B.s) return { t: fld.t, fast: false, s: true, loc: F => [fb(F), nm] };
      return { t: fld.t, fast: false, s: false, loc: function* (F) { return [yield* fb(F), nm]; } };
    }
  ...
  compileUpdate(n) {
    ...
    const w = k === 'bool' ? (v => (d > 0 ? 1 : (v ? 1 : 0))) : WRAP[k];
    if (L.fast) {
      const get = L.get, set = L.set;
      return { t: L.t, s: true, f: F => { const o = get(F); const nv = w(o + d); set(F, nv); return pre ? nv : o; } };
    }
    const loc = L.loc;
    const apply = p => { const o = p[0][p[1]]; const nv = w(o + d); p[0][p[1]] = nv; return pre ? nv : o; };
    if (L.s) return { t: L.t, s: true, f: F => apply(loc(F)) };
    return { t: L.t, s: false, f: function* (F) { return apply(yield* loc(F)); } };
  }
  compileAssign(n) {
    ...
    if (n.value.k === 'InitList') {
      ...
      V = this.makeInit(L.t, n.value, n.tok); V.t = L.t;
    ...
    if (n.op === '=') comb = this.convFn(V.t, L.t, n.tok);
      ...
      comb = null; V.comb = (o, v) => c2(fn(o, v));
    }
    ...
    if (L.fast) {
      const get = L.get, set = L.set;
      if (isEq) {
        if (vs) return { t: L.t, s: true, f: F => { const v = comb(vf(F)); set(F, v); return v; } };
        return { t: L.t, s: false, f: function* (F) { const v = comb(yield* vf(F)); set(F, v); return v; } };
      }
      if (vs) return { t: L.t, s: true, f: F => { const v = cmb(get(F), vf(F)); set(F, v); return v; } };
      return { t: L.t, s: false, f: function* (F) { const r = yield* vf(F); const v = cmb(get(F), r); set(F, v); return v; } };
    }
    const loc = L.loc, ls = L.s;
    if (ls && vs) {
      if (isEq) return { t: L.t, s: true, f: F => { const p = loc(F); const v = comb(vf(F)); p[0][p[1]] = v; return v; } };
      return { t: L.t, s: true, f: F => { const p = loc(F); const v = cmb(p[0][p[1]], vf(F)); p[0][p[1]] = v; return v; } };
    }
    return {
      t: L.t, s: false, f: function* (F) {
        const p = ls ? loc(F) : yield* loc(F);
        const r = vs ? vf(F) : yield* vf(F);
  ...
  compileCondExpr(n) {
    ...
    let rt;
    if (ka === kb && (ka !== 'struct' || A.t.def === B.t.def)) rt = A.t.k === 'enum' ? A.t : A.t;
    ...
    if (T.s && A.s && B.s) return { t: rt, s: true, f: F => (ft(F) ? ca(fa(F)) : cb(fb(F))) };
    const ts = T.s, as = A.s, bs = B.s;
    return { t: rt, s: false, f: function* (F) { const c = ts ? ft(F) : yield* ft(F); if (c) return ca(as ? fa(F) : yield* fa(F)); return cb(bs ? fb(F) : yield* fb(F)); } };
  }
  compileIndex(n) {
    ...
    if (kindOf(B.t) === 'str') {
      const get = (s, i) => (i >= 0 && i < s.length ? WRAP.i8(s.charCodeAt(i)) : 0);
      return this.combine2(TY.i8, B, I, get, tok);
    }
    if (B.t.k !== 'array') this.err(`invalid types '${typeName(B.t)}[${typeName(I.t)}]' for array subscript`, tok);
    const rt = subT(B.t);
    const self = this;
    const get = (a, i) => { if (i >= 0 && i < a.length) return a[i]; M.oob(tok, i, a.length); return self.zeroOf(rt); };
    if (B.s && I.s) return { t: rt, s: true, f: F => get(fb(F), fi(F)) };
    const bs = B.s, is = I.s;
    return { t: rt, s: false, f: function* (F) { const a = bs ? fb(F) : yield* fb(F); const i = is ? fi(F) : yield* fi(F); return get(a, i); } };
  }
  compileMember(n) {
    const B = this.compileExpr(n.obj);
    ...
    const fld = this.fieldOf(B.t, n.name, n.tok);
    const nm = n.name;
    return this.combine1(fld.t, B, o => o[nm]);
  }
  compileCast(T, argNode, tok) {
  ...
  compileCall(n) {
    ...
    if (c.k === 'Ident') {
      ...
      if (name === 'F') {
        ...
        return this.K(TY.str, n.args[0].v);
      }
    ...
    if (c.k === 'Member') {
      ...
      if (kindOf(O.t) === 'str') {
        ...
        if (spec.mut) {
          ...
          const get = L.get, set = L.set, rt = TY[spec.ret];
          if (A.every(a => a.s)) return { t: rt, s: true, f: F => { const r = impl(get(F), ...A.map(a => a.f(F))); set(F, r); return 1; } };
          return { t: rt, s: false, f: function* (F) { const v = []; for (const a of A) v.push(a.s ? a.f(F) : yield* a.f(F)); set(F, impl(get(F), ...v)); return 1; } };
        }
  ...
  compileBuiltin(spec, name, argNodes, tok, objE, allowStr) {
    ...
    // compile-time hints
    if ((name === 'analogWrite') && A[0].cv !== undefined && !PWM_PINS.has(A[0].cv)) this.warn(`analogWrite() on pin ${A[0].cv}: not a PWM pin on the Uno (PWM pins are 3, 5, 6, 9, 10, 11), it will only switch fully on/off`, tok);
    if ((name === 'delay') && A[0].cv !== undefined && A[0].cv < 0) this.warn('negative delay() wraps around to about 49 days on a real Uno', tok);
    const impl = spec.make(this.M, objE ? ['obj', ...kinds] : kinds);
    let rk = typeof spec.ret === 'function' ? spec.ret(kinds) : spec.ret;
    ...
    if (!spec.gen && all.every(a => a.s)) {
      const f = all.map(a => a.f);
      switch (f.length) {
        case 0: return { t: rt, s: true, f: () => impl() };
        case 1: { const f0 = f[0]; return { t: rt, s: true, f: F => impl(f0(F)) }; }
        case 2: { const f0 = f[0], f1 = f[1]; return { t: rt, s: true, f: F => impl(f0(F), f1(F)) }; }
        case 3: { const f0 = f[0], f1 = f[1], f2 = f[2]; return { t: rt, s: true, f: F => impl(f0(F), f1(F), f2(F)) }; }
        default: return { t: rt, s: true, f: F => impl(...f.map(x => x(F))) };
      }
    }
    ...
    return {
      t: rt, s: false, f: function* (F) {
        ...
        for (let i = 0; i < all.length; i++) { const a = all[i]; v[i] = a.s ? a.f(F) : yield* a.f(F); }
  ...
  compileStringCtor(n) {
    ...
    const t = A.t;
    return this.combine2(TY.str, A, B, (v, f) => fmtValue(v, t, f), n.tok);
  }
  compileNewPing(args, tok) {
    ...
    const M = this.M, t = { k: 'obj', cls: 'NewPing' };
    if (A.every(a => a.s)) return { t, s: true, f: F => M.newPing(A[0].f(F), A[1].f(F), A[2] ? A[2].f(F) : 500) };
    return { t, s: false, f: function* (F) { const v = []; for (const a of A) v.push(a.s ? a.f(F) : yield* a.f(F)); return M.newPing(v[0], v[1], v[2] === undefined ? 500 : v[2]); } };
  }
  compileBitOp(name, n) {
    ...
    const k = kindOf(L.t); if (!INT_K.has(k)) this.err(`${name}() needs an integer variable`, n.tok);
    const w = WRAP[k], get = L.get, set = L.set, fb = B.f, fv = V ? V.f : null;
    const op = name === 'bitSet' ? (x, b) => x | (1 << b) : name === 'bitClear' ? (x, b) => x & ~(1 << b) : null;
    if (op) return { t: L.t, s: true, f: F => { const v = w(op(get(F), fb(F))); set(F, v); return v; } };
    return { t: L.t, s: true, f: F => { const b = fb(F); const v = w(fv(F) ? (get(F) | (1 << b)) : (get(F) & ~(1 << b))); set(F, v); return v; } };
  }
  pickOverload(name, n) {
    const recs = this.funcs.get(name);
    const nargs = n.args.length;
    let cands = recs.filter(r => nargs >= r.minArgs && nargs <= r.params.length);
    if (!cands.length) {
      const r = recs[0];
    ...
    const kinds = n.args.map(a => { try { return kindOf(this.compileExpr(a).t); } catch (e) { return '?'; } });
    this.strRam = saved;
  ...
  compileUserCall(name, n) {
    ...
    for (let i = 0; i < rec.params.length; i++) {
      ...
      if (p.isRef) {
        ...
        if (kindOf(L.t) !== kindOf(p.t) || (L.t.k === 'struct' && L.t.def !== p.t.def)) this.err(`cannot bind reference of type '${typeName(p.t)}&' to a value of type '${typeName(L.t)}'`, argNode.tok);
        args.push(L.fast ? { s: true, f: L.pair } : { s: L.s, f: L.loc });
      } else if (p.t.k === 'array') {
        ...
        else if (E.s) args.push({ s: true, f: conv === ID ? ef : F => conv(ef(F)) });
        else args.push({ s: false, f: function* (F) { return conv(yield* ef(F)); } });
      }
    }
    ...
    if (rec.isGen !== false) {
      return {
        t: rt, s: false, f: function* (F) {
          ...
          for (let i = 0; i < na; i++) { const a = args[i]; av[i] = a.s ? a.f(F) : yield* a.f(F); }
    ...
    if (allSync) {
      ...
      if (na === 1) { const a0 = args[0].f; return { t: rt, s: true, f: F => rec.invokeSync([a0(F)]) }; }
      if (na === 2) { const a0 = args[0].f, a1 = args[1].f; return { t: rt, s: true, f: F => rec.invokeSync([a0(F), a1(F)]) }; }
      return { t: rt, s: true, f: F => { const av = new Array(na); for (let i = 0; i < na; i++) av[i] = args[i].f(F); return rec.invokeSync(av); } };
    }
    return {
      t: rt, s: false, f: function* (F) {
        const av = new Array(na);
  ...
  makeInit(t, init, tok) {
    ...
    if (init.k === 'InitList') {
      if (t.k === 'array') {
        const st = subT(t);
        if (init.items.length > t.dims[0]) this.err(`too many initializers for '${typeName(t)}'`, init.tok);
        ...
        if (items.every(x => x.cv !== undefined)) {
          const tmpl = self.zeroOf(t); items.forEach((x, i) => { tmpl[i] = x.cv; });
          return { s: true, f: () => cloneVal(tmpl), cv: tmpl };
        }
        return { s: true, f: F => { const a = self.zeroOf(t); for (let i = 0; i < items.length; i++) a[i] = items[i].f(F); return a; } };
      }
      if (t.k === 'struct') {
        ...
        if (!items.every(x => x.s)) this.err('function calls that use delay()/pulseIn() are not allowed inside { } initializers', init.tok);
        const self = this;
        return { s: true, f: F => { const o = self.zeroOf(t); for (let i = 0; i < items.length; i++) o[fields[i].name] = items[i].f(F); return o; } };
      }
      if (init.items.length === 0) return { s: true, f: () => this.zeroOf(t), cv: this.zeroOf(t) };
    ...
    const ef = E.f;
    if (E.cv !== undefined) { const v = conv(E.cv); if (t.k === 'struct') return { s: true, f: () => cloneVal(v) }; return { s: true, f: () => v, cv: v, et: E.t }; }
    if (E.s) return { s: true, f: F => conv(ef(F)), et: E.t };
    return { s: false, f: function* (F) { return conv(yield* ef(F)); }, et: E.t };
  }
  compileVarDecl(d, isGlobal) {
    const G = this.G, M = this.M;
    const baseT = this.resolveType(d.type, d.tok);
    const isConst = !!d.type.isConst, isStatic = !!d.type.isStatic && !isGlobal;
    const steps = [];
    for (const dc of d.declarators) {
      ...
      if (t.k === 'obj') {
        ...
        else if (init) { mk = this.compileExpr(init); if (mk.t.k !== 'obj') this.err(`cannot convert '${typeName(mk.t)}' to 'NewPing'`, dc.tok); }
      ...
      if (mk && mk.cv !== undefined && init && (init.k === 'Num' || (init.k === 'Unary' && init.a.k === 'Num')) && INT_K.has(kindOf(t)) && t.k !== 'bool') {
      ...
      if (isGlobal || isStatic) {
        ...
        const sym = { kind: 'global', t, slot, isConst };
          ...
          if (mk) {
            if (mk.cv !== undefined && t.k !== 'array' && t.k !== 'struct') G[slot] = mk.cv;
              ...
              if (isStatic) {
                ...
                const saved = this.fn; this.fn = null; this.fn = saved;
                this.initSteps.push({ s: true, f: F => { G[slot] = f(F); return 0; } });
              } else if (mk.s) steps.push({ s: true, f: F => { G[slot] = f(F); return 0; } });
              else steps.push({ s: false, f: function* (F) { G[slot] = yield* f(F); return 0; } });
            }
          }
        }
        ...
        const sym = { kind: 'local', t, slot, isConst };
        const scalar = !(t.k === 'array' || t.k === 'struct' || t.k === 'obj');
        if (isConst && scalar && mk && mk.cv !== undefined) sym.cv = mk.cv;
        const self = this;
        if (!mk) {
          if (t.k === 'array' || t.k === 'struct') steps.push({ s: true, f: F => { F[slot] = self.zeroOf(t); return 0; } });
          else { const z = this.zeroOf(t); steps.push({ s: true, f: F => { F[slot] = z; return 0; } }); }
        } else {
          const f = mk.f;
          if (mk.s) steps.push({ s: true, f: F => { F[slot] = f(F); return 0; } });
          else steps.push({ s: false, f: function* (F) { F[slot] = yield* f(F); return 0; } });
        }
        this.declare(dc.name, sym, dc.tok);
      }
    }
  ...
  compileExprQuiet(n) { const saved = this.strRam; try { const e = this.compileExpr(n); return e.cv; } catch (e) { return undefined; } finally { this.strRam = saved; } }
  defineEnum(def, scope) {
    const map = new Map(); let next = 0;
    const et = { k: 'enum', name: def.name, def };
  ...
  seq(list) {
    ...
    if (!list.length) return { s: true, f: () => 0 };
    if (list.length === 1) return list[0];
    const n = list.length;
    if (list.every(x => x.s)) return { s: true, f: F => { for (let i = 0; i < n; i++) { const r = list[i].f(F); if (r) return r; } return 0; } };
    return { s: false, f: function* (F) { for (let i = 0; i < n; i++) { const st = list[i]; const r = st.s ? st.f(F) : yield* st.f(F); if (r) return r; } return 0; } };
  }
  ...
  compileStmt(n) {
    switch (n.k) {
      ...
      case 'Expr': {
        ...
        if (E.s) return { s: true, f: F => { f(F); return 0; } };
        return { s: false, f: function* (F) { yield* f(F); return 0; } };
      }
      ...
      case 'If': {
        ...
        this.pushScope(); const C = this.compileStmt(n.cons) || { s: true, f: () => 0 }; this.popScope();
        let A = null; if (n.alt) { this.pushScope(); A = this.compileStmt(n.alt); this.popScope(); }
        const ft = T.f, fc = C.f, fa = A ? A.f : null;
        if (T.s && C.s && (!A || A.s)) return { s: true, f: fa ? F => (ft(F) ? fc(F) : fa(F)) : F => (ft(F) ? fc(F) : 0) };
        const ts = T.s, cs = C.s, as = A ? A.s : true;
        return { s: false, f: function* (F) { const c = ts ? ft(F) : yield* ft(F); if (c) return cs ? fc(F) : yield* fc(F); if (!fa) return 0; return as ? fa(F) : yield* fa(F); } };
      }
      case 'While': case 'DoWhile': {
        const T = this.condExpr(n.test);
        this.fn.ctx.push('loop'); this.pushScope();
        const B = this.compileStmt(n.body) || { s: true, f: () => 0 };
        this.popScope(); this.fn.ctx.pop();
        const tail = this.loopTail();
        const ft = T.f, fb = B.f, ts = T.s, bs = B.s, isDo = n.k === 'DoWhile';
        return {
          s: false, f: function* (F) {
            let first = isDo;
            while (true) {
              if (!first) { const c = ts ? ft(F) : yield* ft(F); if (!c) return 0; }
              first = false;
              const r = bs ? fb(F) : yield* fb(F);
      ...
      case 'For': {
        ...
        return {
          s: false, f: function* (F) {
            if (fi) { if (is) fi(F); else yield* fi(F); }
            while (true) {
              if (ft) { const c = ts ? ft(F) : yield* ft(F); if (!c) return 0; }
              const r = bs ? fb(F) : yield* fb(F);
              if (r === SIG_BRK) return 0;
              if (r === SIG_RET) return SIG_RET;
              if (fu) { if (us) fu(F); else yield* fu(F); }
              if (tail()) yield 0;
            }
          }
        };
      ...
      case 'Switch': {
        ...
        if (!INT_K.has(kindOf(D.t))) this.err(`switch quantity not an integer (it is '${typeName(D.t)}')`, n.tok);
        this.fn.ctx.push('switch'); this.pushScope();
        const flat = []; const map = new Map(); let def = -1;
        for (const c of n.cases) {
          ...
          for (const s of c.body) { const st = this.compileStmt(s); flat.push(st || { s: true, f: () => 0 }); }
        }
        this.popScope(); this.fn.ctx.pop();
        ...
        if (allSync) return {
          s: true, f: F => {
            const v = fd(F); let i = map.has(v) ? map.get(v) : def; if (i < 0) return 0;
            for (; i < nf; i++) { const r = flat[i].f(F); if (r === SIG_BRK) return 0; if (r) return r; } return 0;
          }
        };
        return {
          s: false, f: function* (F) {
            const v = ds ? fd(F) : yield* fd(F); let i = map.has(v) ? map.get(v) : def; if (i < 0) return 0;
            for (; i < nf; i++) { const st = flat[i]; const r = st.s ? st.f(F) : yield* st.f(F); if (r === SIG_BRK) return 0; if (r) return r; } return 0;
          }
        };
      }
      case 'Break': if (!this.fn.ctx.length) this.err('break statement not within loop or switch', n.tok); return { s: true, f: () => SIG_BRK };
      case 'Continue': if (!this.fn.ctx.includes('loop')) this.err('continue statement not within a loop', n.tok); return { s: true, f: () => SIG_CNT };
      case 'Return': {
        ...
        if (E.s) return { s: true, f: F => { F[0] = conv(ef(F)); return SIG_RET; } };
        return { s: false, f: function* (F) { F[0] = conv(yield* ef(F)); return SIG_RET; } };
      }
    }
    this.err('unsupported statement', n.tok);
  }
  ...
  declareFunc(d) {
    ...
    if (rec) {
      ...
      rec = { name: d.name, retT, params, sig, node: d.k === 'Func' ? d : null, tok: d.tok, isGen: undefined, pslots: [], nSlots: 1, body: null };
      const M = this.M;
      rec.retDefault = this.zeroOf(retT.k === 'array' ? TY.i16 : retT);
      rec.invokeSync = av => {
        const F = new Array(rec.nSlots);
      ...
      rec.invokeGen = function* (av) {
        const F = new Array(rec.nSlots);
        for (let i = 0; i < av.length; i++) F[rec.pslots[i]] = av[i];
        if (++M.depth > 150) M.stackOverflow(rec.name);
        M.t += 0.5;
        if (rec.body.s) rec.body.f(F); else yield* rec.body.f(F);
        M.depth--;
        const r = F[0]; return r === undefined ? rec.retDefault : r;
      };
  ...
  analyzeGen() {
    ...
    const visit = (node, inf, depth) => {
      ...
      switch (node.k) {
        ...
        case 'Call': {
          const c = node.callee;
          if (c.k === 'Ident') { if (this.funcs.has(c.name)) inf.callees.add(c.name); else if (GEN_BUILTINS.has(c.name)) inf.own = true; }
          if (c.k === 'Member' && GEN_METHODS.has(c.name)) inf.own = true;
          break;
        }
      }
    ...
    for (const [name, recs] of this.funcs) for (const r of recs) {
      const inf = { own: false, callees: new Set() };
  ...
  compileFuncBody(rec) {
    ...
    rec.params.forEach(p => {
      ...
      if (p.name) {
        ...
        this.fn.scopes[0].set(p.name, { kind: p.isRef ? 'ref' : 'local', t: p.t, slot });
      }
    });
    const body = this.compileStmt(d.body) || { s: true, f: () => 0 };
  ...
  compile() {
    ...
    for (const [, recs] of this.funcs) for (const r of recs) if (!r.node) {
      const used = true;
      if (used) { r.body = { s: true, f: () => 0 }; r.missing = true; }
    }
    const find = nm => (this.funcs.get(nm) || []).find(r => r.params.length === 0 && r.node);
...
function compileSketch(files, mainName, M) {
...
export { PURE_BUILTINS, ID, EMPTY_ARGS, valueRange, ArduinoCompiler, compileSketch };
