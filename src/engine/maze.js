...
class Maze {
  constructor(cols, rows) {
    ...
    this.h = new Uint8Array((rows + 1) * cols);   // horizontal edges: h[r*cols+c] is the wall at y=r between x=c..c+1
    this.v = new Uint8Array(rows * (cols + 1));   // vertical edges:   v[r*(cols+1)+c] is the wall at x=c between y=r..r+1
  ...
  fill(val) { this.h.fill(val ? 1 : 0); this.v.fill(val ? 1 : 0); }
  border() {
    const C = this.cols, R = this.rows;
    for (let c = 0; c < C; c++) { this.h[c] = 1; this.h[R * C + c] = 1; }
    for (let r = 0; r < R; r++) { this.v[r * (C + 1)] = 1; this.v[r * (C + 1) + C] = 1; }
  }
  isBorderEdge(kind, idx) {
    const C = this.cols, R = this.rows;
    if (kind === 'h') { const r = Math.floor(idx / C); return r === 0 || r === R; }
    const c = idx % (C + 1); return c === 0 || c === C;
  }
  toJSON() { return { cols: this.cols, rows: this.rows, h: Array.from(this.h).join(''), v: Array.from(this.v).join(''), start: this.start, goal: this.goal }; }
  static fromJSON(o) {
  ...
  deadEnds() { const out = []; for (let r = 0; r < this.rows; r++) for (let c = 0; c < this.cols; c++) { let n = 0; for (let d = 0; d < 4; d++) n += this.wall(c, r, d); if (n === 3) out.push([c, r]); } return out; }
}

function genBacktracker(m, rng) {
  m.fill(1);
...
function genPrim(m, rng) {
  m.fill(1);
  const inMaze = new Uint8Array(m.cols * m.rows);
  const frontier = [];
  const add = (c, r) => { inMaze[r * m.cols + c] = 1; for (let d = 0; d < 4; d++) { const nc = c + DX[d], nr = r + DY[d]; if (m.inside(nc, nr) && !inMaze[nr * m.cols + nc]) frontier.push([c, r, d]); } };
...
function generateMaze(type, cols, rows, seed, goalMode) {
...
export { DX, Maze, genBacktracker, genPrim, braidMaze, generateMaze, placeGoal, DY };
