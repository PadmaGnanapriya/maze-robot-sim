...
class World {
  constructor() {
    ...
    this.wl = 0; this.wr = 0; this.angL = 0; this.angR = 0;
    this.t = 0; this.mcu = null; this.held = false;
    this.contacts = 0; this.inContact = false; this.lastContactT = -1e12; this.contactFlash = 0;
    this.trail = []; this.trailDist = 0;
    this.goalReached = false; this.onGoal = null; this.onContact = null;
    this.readings = [null, null, null];
    this.walls = []; this.buckets = null;
    this.casterAng = 0; this.odo = 0;
    this.rng = mulberry32(99);
  }
  setRobot(cfg) { this.cfg = Object.assign({}, DEFAULT_ROBOT, cfg); this.geo = robotGeometry(this.cfg); }
  setMaze(maze, cell, wallT) {
    this.maze = maze; this.cell = cell; this.wallT = wallT;
    const walls = []; const C = maze.cols, R = maze.rows, h = wallT / 2;
  ...
  nearWalls(x, y) {
    ...
    return this.buckets[r * this.maze.cols + c];
  }
  startPose() {
  ...
  chassisCenter(p) { p = p || this.pose; const c = this.geo.cx; return { x: p.x + Math.cos(p.th) * c, y: p.y + Math.sin(p.th) * c }; }
  setChassisCenter(x, y, th) { const c = this.geo.cx; this.pose = { x: x - Math.cos(th) * c, y: y - Math.sin(th) * c, th }; }
  syncTo(t) {
    let guard = 0;
  ...
  motorStep(dt) {
    ...
    for (let side = 0; side < 2; side++) {
      const cmd = this.mcu ? this.mcu.motorCmd(side) : { duty: 0, brake: 0, coast: true };
    ...
    for (let iter = 0; iter < 3; iter++) {
      ...
      for (const ci of this.geo.circles) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          let dx = px - qx, dy = py - qy; const d2 = dx * dx + dy * dy;
  ...
  resolveOverlap() { this.collide(); this.collide(); this.collide(); }
  rayCast(ox, oy, dx, dy, maxD) {
    ...
    for (let k = 0; k < walls.length; k++) {
      ...
      let txn = t1 < t2 ? t1 : t2, txf = t1 < t2 ? t2 : t1;
...
export { DEFAULT_ROBOT, DEFAULT_WIRING, robotGeometry, World };
